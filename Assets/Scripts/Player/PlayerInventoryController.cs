...
public class PlayerInventoryController : MonoBehaviour
{
    ...
    void Update()
    {
        if (_playerPillController.CanTakePill())
        {
            ...
            {
                RemovePill(PlayerPillController.PillType.Enlarge);
            }
            if (Input.GetKeyDown(KeyCode.Alpha2) && (_inventoryDelargePillNum >= 1))
            {
                RemovePill(PlayerPillController.PillType.Delarge);
            }
    ...
    public bool CanInventoryPill(Pill pill)
    {
        ...
        {
            case PlayerPillController.PillType.Enlarge:
                _canInventoryPill = (_inventoryEnlargePillNum < _maxInventoryEnlargePill);
                return _canInventoryPill;
            case PlayerPillController.PillType.Delarge:
    ...
    private Pill GetEnlargePill()
    {
        return new Pill
        {
            type = PlayerPillController.PillType.Enlarge,
            duration = 5,
            startFlashTime = 2,
            flashInterval = .3f,
            targetXScale = 2,
    ...
    private Pill GetDelargePill()
    {
        return new Pill
        {
            type = PlayerPillController.PillType.Delarge,
            duration = 5,
            startFlashTime = 2,
            flashInterval = .3f,
            targetXScale = .4f,
